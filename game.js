...
const world={minX:-1500,minY:-3900,w:30450,h:1100};
...
const currentStreams=[
 ...
 {id:'upperExpress',source:'road',cloudX:14020,cloudY:-70,width:68,speed:235,frozen:0,visibleRoad:true,
...
const flyingVeg=[]; // P138: 空からのタネ攻撃は撤去。飛行野菜も出さない。
...
const fruitRoadGeo={path:[
 {x:8580,y:-2555,w:720,h:210},
 {x:9250,y:-2620,w:720,h:210},
 {x:9920,y:-2545,w:720,h:210},
 {x:10590,y:-2620,w:720,h:210},
 {x:11260,y:-2555,w:900,h:210}
]};
const roadFlowers=[
 {x:8810,y:-2745,r:25,hp:7,maxHp:7,attackCd:.6,flash:0,dead:false},
 {x:9160,y:-2200,r:25,hp:7,maxHp:7,attackCd:1.0,flash:0,dead:false},
 {x:9510,y:-2810,r:25,hp:7,maxHp:7,attackCd:.8,flash:0,dead:false},
 {x:9860,y:-2180,r:25,hp:7,maxHp:7,attackCd:1.2,flash:0,dead:false},
 {x:10210,y:-2740,r:25,hp:7,maxHp:7,attackCd:.7,flash:0,dead:false},
 {x:10560,y:-2205,r:25,hp:7,maxHp:7,attackCd:1.1,flash:0,dead:false},
 {x:10910,y:-2810,r:25,hp:7,maxHp:7,attackCd:.9,flash:0,dead:false},
 {x:11260,y:-2200,r:25,hp:7,maxHp:7,attackCd:1.3,flash:0,dead:false},
 {x:11610,y:-2740,r:25,hp:7,maxHp:7,attackCd:.8,flash:0,dead:false}
];
const roadVegSoldiers=[
 {type:'carrotSword',x:9000,y:-2450,r:27,hp:11,maxHp:11,speed:62,attackCd:.6,flash:0,dead:false},
 {type:'leekSpear',x:9370,y:-2515,r:29,hp:12,maxHp:12,speed:58,attackCd:.8,flash:0,dead:false},
 {type:'turnipHammer',x:9750,y:-2440,r:32,hp:14,maxHp:14,speed:50,attackCd:1.0,flash:0,dead:false},
 {type:'carrotSword',x:10150,y:-2460,r:27,hp:11,maxHp:11,speed:64,attackCd:.7,flash:0,dead:false},
 {type:'leekSpear',x:10520,y:-2515,r:29,hp:12,maxHp:12,speed:60,attackCd:.9,flash:0,dead:false},
 {type:'turnipHammer',x:10920,y:-2460,r:32,hp:14,maxHp:14,speed:52,attackCd:1.1,flash:0,dead:false},
 {type:'carrotSword',x:11320,y:-2450,r:27,hp:12,maxHp:12,speed:66,attackCd:.8,flash:0,dead:false},
 {type:'leekSpear',x:11710,y:-2470,r:29,hp:13,maxHp:13,speed:60,attackCd:1.0,flash:0,dead:false}
];
const fruitMiniBosses=[
 {type:'apple',name:'リンゴ隊長',x:9550,y:-2470,r:52,hp:26,maxHp:26,attackCd:1.0,flash:0,dead:false,active:false,windup:0,thrustT:0,side:-1},
 {type:'orange',name:'オレンジ親分',x:10320,y:-2460,r:55,hp:30,maxHp:30,attackCd:1.1,flash:0,dead:false,active:false,windup:0,thrustT:0,side:-1},
 {type:'grape',name:'ブドウ団長',x:11100,y:-2470,r:58,hp:34,maxHp:34,attackCd:1.2,flash:0,dead:false,active:false,windup:0,thrustT:0,side:-1}
];
const roadFlowerPads=roadFlowers.map((f,i)=>({
...
const ultraVines=[
 {x:20720,y:-2470,r:46,len:270,a:1.48,hp:8,maxHp:8,dead:false,regenT:0,phase:0,hurtCd:0},
 {x:21050,y:-2470,r:48,len:285,a:1.66,hp:8,maxHp:8,dead:false,regenT:0,phase:1,hurtCd:0},
 {x:21400,y:-2470,r:48,len:280,a:1.50,hp:9,maxHp:9,dead:false,regenT:0,phase:2,hurtCd:0},
 {x:21730,y:-2470,r:48,len:295,a:1.64,hp:9,maxHp:9,dead:false,regenT:0,phase:3,hurtCd:0},
 {x:22080,y:-2470,r:50,len:290,a:1.49,hp:10,maxHp:10,dead:false,regenT:0,phase:4,hurtCd:0},
 {x:22410,y:-2470,r:50,len:300,a:1.65,hp:10,maxHp:10,dead:false,regenT:0,phase:5,hurtCd:0}
];
...
const lateRainbows=[
 {x1:20185,y1:-2470,x2:20385,y2:-2470,w:112}, // 水晶樹王→光花庭園
 {x1:23195,y1:-2470,x2:23510,y2:-2470,w:112}  // 光花庭園→中ボスラッシュ
];
...
const rushMiniBosses=[
 {type:'melon',name:'メロン騎士',x:23820,y:-2470,r:48,hp:24,maxHp:24,attackCd:.6,dead:false,flash:0,windup:0,thrustT:0,side:-1},
 {type:'berry',name:'ベリー将軍',x:24460,y:-2470,r:50,hp:28,maxHp:28,attackCd:.8,dead:false,flash:0,windup:0,thrustT:0,side:-1},
 {type:'pine',name:'パイン重兵',x:25100,y:-2470,r:54,hp:32,maxHp:32,attackCd:1.0,dead:false,flash:0,windup:0,thrustT:0,side:-1},
 {type:'citrus',name:'柑橘剣士',x:25720,y:-2470,r:52,hp:36,maxHp:36,attackCd:.7,dead:false,flash:0,windup:0,thrustT:0,side:-1}
];
...
// 光の杖入手後の最終空路。小島を光翼で渡り、丸い最終ボス島へ。
const finalSkyRoute={
 arrows:[
  {x:26720,y:-2470,a:0},{x:27150,y:-2640,a:-.22},{x:27620,y:-2360,a:.18},
  {x:28120,y:-2630,a:-.16},{x:28620,y:-2420,a:.10}
 ],
 stepping:[
  {x:26940,y:-2700,r:105},{x:27380,y:-2300,r:112},{x:27840,y:-2700,r:108},
  {x:28320,y:-2290,r:115},{x:28720,y:-2620,r:110}
 ],
 arena:{x:29320,y:-2470,r:360},
 safe:[
  {x:29320,y:-3020,r:135},{x:29900,y:-2470,r:135},{x:29320,y:-1915,r:135},{x:28740,y:-2470,r:135}
 ]
};
const finalBoss={
 ...
 if(fireBossDefeated||raceRouteOpen()){
   ...
   if(raceRouteOpen()){
     ...
     if(cloudRaceWon){
       ...
       if(crystalBossDefeated){grounds.push(...lightGardenGeo.path);const rb=lateRainbows[0];grounds.push({x:Math.min(rb.x1,rb.x2)-18,y:rb.y1-rb.w/2,w:Math.abs(rb.x2-rb.x1)+36,h:rb.w});}
       if(lightBossDefeated){grounds.push(...bananaRushGeo.path);const rb=lateRainbows[1];grounds.push({x:Math.min(rb.x1,rb.x2)-18,y:rb.y1-rb.w/2,w:Math.abs(rb.x2-rb.x1)+36,h:rb.w});}
       if(player.lightStaff||bananaBossDefeated){
         for(const q of finalSkyRoute.stepping)grounds.push({x:q.x-q.r,y:q.y-q.r,w:q.r*2,h:q.r*2});
         const ar=finalSkyRoute.arena;grounds.push({x:ar.x-ar.r,y:ar.y-ar.r,w:ar.r*2,h:ar.r*2});
         for(const q of finalSkyRoute.safe)grounds.push({x:q.x-q.r,y:q.y-q.r,w:q.r*2,h:q.r*2});
       }
     }
   }
 }
...
function enemySupportedByGround(e,pad=10){
 ...
 if(vegBossDefeated && typeof fruitRoadGeo!=='undefined'){
...
function enforceAllGroundEnemies(dt){
 const lists=[enemies,stage2Enemies,stage3Enemies,stage4Enemies,stage6Enemies,stage8Enemies,vegArmy,roadVegSoldiers];
...
function skill(){
 ...
 if(player.weapon===0){
   ...
   player.skillKind='lightWing';player.skillT=3.0;player.lightWingT=3.0;player.lightTerrainCd=0;
...
function staffCrystalBoost(){
 return !!shields[player.shieldType]?.crystalMagic;
}
function lightAllEnemies(){
 ...
 for(const list of [enemies,stage2Enemies,stage3Enemies,stage4Enemies,stage6Enemies,stage8Enemies,stage10Enemies,
   iceEnemies,iceThrowers,vineSeedFlowers,whipVines,bossWalnuts,vegArmy,vegFireFlowers,flyingVeg,roadVegSoldiers,
   roadFlowers,fruitMiniBosses,waterRaiders,lavaThrowers,crystalPlants,crystalThrowers,rapidFlowers,ultraVines,
   walkingGrass,rushFlowers,rushMiniBosses])for(const e of list)if(e&&!e.dead)out.push(e);
 for(const b of [boss,seedBoss,grassFinalBoss,hammerGuardian,rockBoss,islandBoss,fireBoss,iceBoss,vineBoss,vegBoss,
   fruitSpearBoss,waterBoss,lavaBoss,crystalBoss,lightBoss,bananaBoss,finalBoss])if(b&&!b.dead&&(!('active' in b)||b.active))out.push(b);
 if(stage10Started&&!rockThrower.dead)out.push(rockThrower);
 return out;
}
function lightBreakTerrainAt(x,y,r=60){
 const near=(o,extra=0)=>o&&!o.dead&&dist(x,y,o.x,o.y)<r+(o.r||extra||28);
 for(const o of iceRouteBlocks)if(near(o))o.dead=true;
 for(const o of crystalIceBlocks)if(near(o))o.dead=true;
 for(const o of elementalObstacles)if(near(o))o.dead=true;
 for(const o of props.rocks)if(near(o,34))o.dead=true;
 for(const o of stage7Rocks)if(near(o))o.dead=true;
 for(const o of hammerGuardianRocks)if(near(o))o.dead=true;
 for(const o of rollingRocks)if(near(o))o.dead=true;
 for(const g of props.grass)if(near(g,24))g.dead=true;
 for(const tr of props.smallTrees)if(near(tr,34))tr.dead=true;
 for(const v of vineWalls)if(near(v)&&!v.perma){v.dead=true;v.regenT=5;v.burned=true;}
 for(const v of vineKnot)if(near(v)&&!v.perma){v.dead=true;v.regenT=5;}
 for(const v of ultraVines){
   if(v.dead)continue;
   if(dist(x,y,v.x,v.y)<r+(v.len||220)*.55){v.hp=0;v.dead=true;v.regenT=2.2;}
 }
}
...
function lightLineHit(x,y,a,len,width,damage){
...
function fireMagic(w,charged,base){
 ...
 const crystal=staffCrystalBoost();
 ...
 if(charged){
   ...
   // 杖のチャージ攻撃は追加された後半敵も含め、現在存在する全戦闘対象へ同じ判定を使う。
   for(const e of lightAllEnemies())hitOne(e);
...
function hitVegArea(damage,range,base,cone,weapon){
 ...
 if(finalBoss.active&&!finalBoss.dead){const d=dist(player.x,player.y,finalBoss.x,finalBoss.y),a=Math.atan2(finalBoss.y-player.y,finalBoss.x-player.x);if(d<=range+finalBoss.r+20&&Math.abs(angleDiff(a,base))<=cone/2+.2){finalBoss.hp-=damage;finalBoss.flash=.2;}}
...
function doAttack(charged=false){
 ...
 if(w===5){
   ...
   if(charged){
     ...
     lightLineHit(player.x,player.y,base,850,34,dmg);
 ...
 if(jumpStrike){
   ...
   if(w===0||w===1){
     ...
     player.airMagic={
       kind:w===3?'fire':'ice', crystalTriple:staffCrystalBoost(),
 ...
 // 後半追加敵も「通常攻撃」の共通対象にする。特に氷から出た crystalThrower の剣通常漏れを修正。
 if(w<=2){
   const lateHit=(e)=>{
     if(!e||e.dead)return;const d=dist(player.x,player.y,e.x,e.y),aa=Math.atan2(e.y-player.y,e.x-player.x);
     if(d<=range+(e.r||22)&&Math.abs(angleDiff(aa,base))<=cone/2+.08){
       e.hp=(e.hp??1)-commonD;e.flash=.2;particle(e.x,e.y-22,`-${commonD}`,'#b31313',.4,15);
       if(e.hp<=0){e.dead=true;if(ultraVines.includes(e))e.regenT=.72;}
     }
   };
   for(const list of [crystalPlants,crystalThrowers,rapidFlowers,ultraVines,walkingGrass,rushFlowers,rushMiniBosses])for(const e of list)lateHit(e);
 }
...
function releaseShieldEnergy(base){
 ...
 for(const list of [enemies,stage2Enemies,stage3Enemies,stage4Enemies,stage6Enemies,stage8Enemies,stage10Enemies,iceEnemies,iceThrowers,vineSeedFlowers,whipVines,bossWalnuts,vegArmy,vegFireFlowers,flyingVeg,roadVegSoldiers,roadFlowers,fruitMiniBosses])for(const e of list)hit(e);
 for(const b of [boss,seedBoss,grassFinalBoss,hammerGuardian,rockBoss,islandBoss,fireBoss,iceBoss,vineBoss,vegBoss,fruitSpearBoss]){
...
function resolveAirMagic(m){
 if(!m||m.done)return;
 m.done=true;
 const boosted=!!m.crystalTriple;
 const rad=boosted?108:88,dmg=boosted?8:5;
 const centers=[{x:m.x,y:m.y}];

 if(boosted){
   const a=Math.atan2(m.y-player.y,m.x-player.x),px=-Math.sin(a),py=Math.cos(a);
   centers.push({x:m.x+px*82,y:m.y+py*82},{x:m.x-px*82,y:m.y-py*82});
 }

 particle(m.x,m.y,m.kind==='fire'?'ボォン！':'キィン！',m.kind==='fire'?'#e43':'#268bc1',.55,20);
 for(const c of centers){
   airMagicImpacts.push({
     x:c.x,y:c.y,kind:m.kind,
     life:m.kind==='ice'?.92:.52,max:m.kind==='ice'?.92:.52,
     flake:m.kind==='ice'
   });
 }
 ...
 const hit=(e)=>{
   ...
   let inside=false;
   for(const c of centers){
     if(dist(c.x,c.y,e.x,e.y)<rad+(e.r||20)){inside=true;break;}
   }
   if(!inside)return;
   e.hp=(e.hp??1)-dmg;e.flash=.22;
   particle(e.x,e.y-25,`-${dmg}`,m.kind==='fire'?'#e43':'#268bc1',.4,16);
   if(e.hp<=0){
     e.dead=true;
     if(ultraVines.includes(e))e.regenT=m.kind==='fire'?2.4:.9;
   }
 };

 // 赤杖・青杖のジャンプ着弾は、後半追加敵・中ボス・ラスボスまで共通で当たる。
 for(const e of lightAllEnemies())hit(e);

 if(m.kind==='fire'){
   for(const c of centers){
     for(const g of props.grass){
       if(!g.dead&&dist(c.x,c.y,g.x,g.y)<105){g.dead=true;particle(g.x,g.y,'ボワッ','#e43')}
     }
     for(const o of iceRouteBlocks){
       if(!o.dead&&dist(c.x,c.y,o.x,o.y)<105){o.dead=true;particle(o.x,o.y,'ジュワッ！','#bfeeff',.4,15)}
     }
     meltCrystalIceAt(c.x,c.y,112);
     // 再生ツタ類も火柱の範囲にいれば燃やして破壊。
     for(const v of [...vineWalls,...vineKnot]){
       if(v.dead||v.perma||dist(c.x,c.y,v.x,v.y)>=rad+(v.r||30))continue;
       v.hp=(v.hp??1)-dmg;
       if(v.hp<=0){v.dead=true;v.burned=true;v.regenT=5;particle(v.x,v.y,'ボワッ！','#e43',.45,16);}
     }
   }
 }else{
   for(const c of centers){
     freezeLegacyWaterAt(c.x,c.y,105);
     freezeStreamsAt(c.x,c.y,105);
     freezeLavaAt(c.x,c.y,118);
   }
 }
...
function update(dt){
 ...
 if(player.hammerSmash>0){
   ...
   if(p>=.985&&!player.smashHit){
     ...
     for(const b of [boss,seedBoss,grassFinalBoss,hammerGuardian,rockBoss,islandBoss,fireBoss,iceBoss,vineBoss,vegBoss,fruitSpearBoss,waterBoss,lavaBoss,crystalBoss,lightBoss,bananaBoss,finalBoss,...fruitMiniBosses,...rushMiniBosses]){
 ...
 if(player.skillT>0){
   ...
   if(player.skillKind==='sword'){
     ...
     player.lightTerrainCd=(player.lightTerrainCd??0)-dt;
     if(player.lightTerrainCd<=0){player.lightTerrainCd=.10;lightBreakTerrainAt(player.x,player.y,92);}
     ...
     for(const e of lightAllEnemies()){
       ...
       if(dist(player.x,player.y,e.x,e.y)<78+(e.r||20)){
         e.hp-=staffCrystalBoost()?6:3;e.flash=.15;player.skillHit.add(e);particle(e.x,e.y-20,'炎！','#e43',.4,15);
         if(e.hp<=0){e.dead=true;if(ultraVines.includes(e))e.regenT=2.4;}
       }
     }
     ...
     for(const e of lightAllEnemies()){
       if(e.dead||player.skillHit.has(e))continue;
       if(dist(player.x,player.y,e.x,e.y)<82+(e.r||20)){
         e.hp-=staffCrystalBoost()?6:3;e.flash=.15;e.stagger=Math.max(e.stagger||0,.55);player.skillHit.add(e);
         particle(e.x,e.y-20,'ガツン！','#268bc1',.4,15);
         if(e.hp<=0){e.dead=true;if(ultraVines.includes(e))e.regenT=.9;}
       }
     }
   }
 }
 ...
 for(const b of [boss,seedBoss,grassFinalBoss,hammerGuardian,rockBoss,islandBoss,fireBoss,iceBoss,vineBoss,vegBoss,fruitSpearBoss,waterBoss,lavaBoss,crystalBoss,lightBoss,bananaBoss,finalBoss]){
 ...
 // 超再生ツタは「壁」。長いツタ本体＋トゲに触れると押し戻されてダメージ。
 for(const v of ultraVines){
   if(v.dead)continue;
   const vd=ultraVineDistance(v,player.x,player.y);
   if(vd<player.r+22){
     player.x=prevX;player.y=prevY;
     if(v.hurtCd<=0&&player.inv<=0){
       v.hurtCd=.7;const got=takeDamage(7);player.inv=.55;
       particle(player.x,player.y-28,`トゲ！ -${got}`,'#8f2d52',.38,15);
     }
   }
 }
 // 再生ツタ壁は壊れている間だけ通れる。
 ...
 for(const st of currentStreams){
   ...
   const roadFlow=!!st.visibleRoad&&!!seg&&visibleHere;
   if(seg&&visibleHere&&(roadFlow||(visibleWater&&sameGround))&&st.frozen<=0){
...
// 崖際の岩・低木は見た目用。通常ステージでは引っかからない。
 ...
 for(const f of player.fireTrail){
   ...
   for(const e of lightAllEnemies()){
     ...
     if(dist(f.x,f.y,e.x,e.y)<30+(e.r||18)*.35){
 ...
 function fireHitAnything(pr){
   for(const e of lightAllEnemies()){
     if(!e||e.dead)continue;
     if(dist(pr.x,pr.y,e.x,e.y)>=pr.r+(e.r||24))continue;
     const dmg=(pr.damage||2)+(e===iceBoss?1:0);
     e.hp=(e.hp??1)-dmg;e.flash=.18;
     particle(e.x,e.y-22,`-${dmg}`,'#e34a3a',.4,15);
     if(e.hp<=0){
       e.dead=true;
       if(ultraVines.includes(e))e.regenT=2.4;
     }
     pr.hit=true;return true;
   }
   return false;
 ...
 function iceHitAnything(pr){
   ...
   const lists=[enemies,stage2Enemies,stage3Enemies,stage4Enemies,stage6Enemies,stage8Enemies,stage10Enemies,
     ...
     rapidFlowers,ultraVines,walkingGrass,rushFlowers,rushMiniBosses];
   ...
   for(const b of [boss,seedBoss,grassFinalBoss,rockBoss,islandBoss,fireBoss,hammerGuardian,iceBoss,vineBoss,vegBoss,fruitSpearBoss,waterBoss,lavaBoss,crystalBoss,lightBoss,bananaBoss,finalBoss]){
 ...
 for(const pr of projectiles){
   ...
   if(pr.hit)continue;
   if(pr.kind==='fire'&&fireHitAnything(pr))continue;
   ...
   if(cloudRaceWon){
     let vh=false;
     for(const list of [vegArmy,vegFireFlowers,flyingVeg,roadVegSoldiers,roadFlowers,fruitMiniBosses]){
       for(const e of list){
         if(e.dead)continue;
   ...
   if(pr.hit)continue;
   if(fruitSpearBoss.active&&!fruitSpearBoss.dead&&dist(pr.x,pr.y,fruitSpearBoss.x,fruitSpearBoss.y)<pr.r+fruitSpearBoss.r){
     fruitSpearBoss.hp-=pr.damage;fruitSpearBoss.flash=.16;pr.hit=true;particle(fruitSpearBoss.x,fruitSpearBoss.y-45,`-${pr.damage}`,'#b31313',.4,16);
   }
   if(pr.hit)continue;
...
// 雲レース：スタート門で2秒カウント後、1/3・2/3の2ゲートを通って1周。
 ...
 if(lightBossDefeated){
  ...
  for(const e of rushMiniBosses){
    if(e.dead)continue;e.flash=Math.max(0,e.flash-dt);e.attackCd-=dt;e.windup=Math.max(0,(e.windup||0)-dt);e.thrustT=Math.max(0,(e.thrustT||0)-dt);
    const d=dist(player.x,player.y,e.x,e.y),a=Math.atan2(player.y-e.y,player.x-e.x);e.side=player.x<e.x?-1:1;
    if(d<400&&d>130){e.x+=Math.cos(a)*30*dt;e.y+=Math.sin(a)*30*dt;}
    if(d<195&&e.attackCd<=0&&e.windup<=0){e.attackCd=1.35;e.windup=.38;e.didHit=false;particle(e.x,e.y-58,'！','#fff',.25,14);}
    if(e.windup>0&&e.windup<=.06&&!e.didHit){e.didHit=true;e.thrustT=.27;if(d<210&&player.jumpT<=0){if(shieldBlocks(e))particle(player.x,player.y-28,'ギィン！','#fff',.3,15);else if(player.inv<=0){const got=takeDamage(7);player.inv=.5;particle(player.x,player.y-25,`槍 -${got}`,'#b31313',.3,15);}}}
    if(e.windup<=0)e.didHit=false;
  }
 ...
 if(player.lightStaff||bananaBossDefeated){
   ...
   if(!fb.dead&&!fb.active&&fd<470){fb.active=true;fb.pulseCd=1.4;say('最終決戦・天空核！ 島全体が光ったら周囲の小島へ飛べ！');}
   if(fb.active&&!fb.dead){
     fb.flash=Math.max(0,fb.flash-dt);fb.pulseCd-=dt;fb.shotCd-=dt;
     if(fb.pulseCd<=0&&fb.pulseWarn<=0){fb.pulseWarn=1.25;fb.pulseHit=false;fb.pulseCd=4.0;particle(fb.x,fb.y-120,'島全体攻撃！','#fff5a8',.75,20);}
     if(fb.pulseWarn>0){
       fb.pulseWarn=Math.max(0,fb.pulseWarn-dt);
       // 発光予告の最後に、丸いボス島全体へ一度だけ大ダメージ。
       if(fb.pulseWarn<=.10&&!fb.pulseHit){
         fb.pulseHit=true;
         const ar=finalSkyRoute.arena;
         if(dist(player.x,player.y,ar.x,ar.y)<ar.r-12){
           const got=takeDamage(22);player.inv=.7;particle(player.x,player.y-45,`天光 -${got}`,'#fff0a0',.55,19);
         }
       }
     }
     // 合間は小さな光弾。盾で凌げる。
 ...
 function ultraVineDistance(v,x,y){
   const ca=Math.cos(v.a||0),sa=Math.sin(v.a||0),hl=(v.len||180)/2;
   const ax=v.x-ca*hl,ay=v.y-sa*hl,bx=v.x+ca*hl,by=v.y+sa*hl;
   const dx=bx-ax,dy=by-ay,l2=dx*dx+dy*dy,t=clamp(((x-ax)*dx+(y-ay)*dy)/l2,0,1);
   return Math.hypot(x-(ax+dx*t),y-(ay+dy*t));
 }
 // 水晶樹王の先：反射を主役にした光花庭園。
 if(crystalBossDefeated){
  for(const v of ultraVines){
   v.phase+=dt*2.2;v.hurtCd=Math.max(0,(v.hurtCd||0)-dt);
 ...
 if(vegBossDefeated){
   for(const f of roadFlowers){
     if(f.dead)continue;f.flash=Math.max(0,f.flash-dt);f.attackCd-=dt;
     const d=dist(player.x,player.y,f.x,f.y);
     if(d<560&&f.attackCd<=0){
       const a=Math.atan2(player.y-f.y,player.x-f.x);
       projectiles.push({x:f.x,y:f.y,vx:Math.cos(a)*205,vy:Math.sin(a)*205,r:10,life:3.0,kind:'seed',damage:5,enemyShot:true,hit:false});
       f.attackCd=1.65;
       particle(f.x,f.y-18,'ポン！','#d9ef82',.22,11);
     }
   }
   for(const e of roadVegSoldiers){
     if(e.dead||e.waitForGround||e.enemyFalling)continue;
     e.flash=Math.max(0,e.flash-dt);e.attackCd-=dt;e.swingT=Math.max(0,(e.swingT||0)-dt);
     const dx=player.x-e.x,dy=player.y-e.y,d=Math.hypot(dx,dy)||1;
     if(d<360&&d>76&&groundEnemyCanChase(e,380)){
       const ox=e.x,oy=e.y;e.x+=dx/d*e.speed*dt;e.y+=dy/d*e.speed*dt;
       if(!enemySupportedByGround(e,4)){e.x=ox;e.y=oy;}
     }
     const reach=e.type==='leekSpear'?116:(e.type==='turnipHammer'?86:78);
     if(d<reach&&e.attackCd<=0){
       e.attackCd=e.type==='turnipHammer'?1.5:1.05;e.swingT=.28;
       if(player.jumpT<=0){
         if(shieldBlocks(e))particle(player.x,player.y-32,'ガキン！','#fff',.3,15);
         else if(player.inv<=0){const got=takeDamage(e.type==='turnipHammer'?7:5);player.inv=.5;particle(player.x,player.y-34,`-${got}`,'#c11',.4,16);}
       }
     }
   }
   for(const b of fruitMiniBosses){
     if(b.dead)continue;b.flash=Math.max(0,b.flash-dt);b.attackCd-=dt;b.windup=Math.max(0,b.windup-dt);b.thrustT=Math.max(0,(b.thrustT||0)-dt);
     const dx=player.x-b.x,dy=player.y-b.y,d=Math.hypot(dx,dy)||1;b.side=dx<0?-1:1;
     if(!b.active&&d<360){b.active=true;say(`${b.name}！`);}
     if(!b.active)continue;
     if(d>125&&d<420){const ox=b.x,oy=b.y;b.x+=dx/d*30*dt;b.y+=dy/d*30*dt;if(!enemySupportedByGround(b,8)){b.x=ox;b.y=oy;}}
     if(d<190&&b.attackCd<=0&&b.windup<=0){b.attackCd=1.55;b.windup=.42;b.didHit=false;particle(b.x,b.y-58,'槍を構えた！','#fff',.25,14);}
     if(b.windup>0&&b.windup<=.07&&!b.didHit){
       b.didHit=true;b.thrustT=.28;
       if(d<205&&player.jumpT<=0){
         if(shieldBlocks(b))particle(player.x,player.y-32,'ギィン！','#ffd34d',.32,16);
         else if(player.inv<=0){const dmg=b.type==='grape'?8:(b.type==='orange'?7:6);const got=takeDamage(dmg);player.inv=.55;particle(player.x,player.y-34,`槍 -${got}`,'#c11',.42,16);}
       }
     }
     if(b.windup<=0)b.didHit=false;
   }
 }
...
function drawSurfaceStreams(){
 for(const st of currentStreams){
   ...
   ctx.save();
   // 上段高速水流は通路そのものに沿う専用表示。地面矩形クリップで消さない。
   if(!st.visibleRoad)clipVisibleGround();
   ctx.globalAlpha=st.visibleRoad?.94:.84;ctx.lineJoin='round';ctx.lineCap='round';
   ctx.strokeStyle=active?(st.visibleRoad?'#219fd0':'#4ca6c8'):'#bdeeff';ctx.lineWidth=st.width+8;traceStreamPath(st);ctx.stroke();
...
function drawUpperExpressCurrent(){
 const st=currentStreams.find(x=>x.id==='upperExpress');
 if(!st)return;
 const active=st.frozen<=0;
 ctx.save();ctx.lineJoin='round';ctx.lineCap='round';
 // 草地の上にはっきり残る水路。縁・水面・流向矢印の3層。
 ctx.globalAlpha=.98;ctx.strokeStyle=active?'#167fae':'#aeeaff';ctx.lineWidth=st.width+14;traceStreamPath(st);ctx.stroke();
 ctx.strokeStyle=active?'#57c8ea':'#dff8ff';ctx.lineWidth=st.width+2;traceStreamPath(st);ctx.stroke();
 ctx.strokeStyle='rgba(255,255,255,.92)';ctx.lineWidth=5;
 const tm=performance.now()/1000*st.speed*.72;
 for(let i=0;i<st.pts.length-1;i++){
   const a=st.pts[i],b=st.pts[i+1],dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy)||1,nx=dx/len,ny=dy/len,px=-ny,py=nx;
   for(let q=30+((tm+i*31)%120);q<len;q+=120){
     const x=a.x+nx*q,y=a.y+ny*q,sz=16;
     // ptsは右→左なので、このV字の先が実際の流れ方向を向く。
     ctx.beginPath();
     ctx.moveTo(x-nx*sz-px*sz*.72,y-ny*sz-py*sz*.72);
     ctx.lineTo(x,y);
     ctx.lineTo(x-nx*sz+px*sz*.72,y-ny*sz+py*sz*.72);
     ctx.stroke();
   }
 }
 ctx.restore();
...
// 第3島：少し濃い草と木の実が多い草原
 ...
 if(rockBossDefeated){
   ...
   if(islandBossDefeated){
     ...
     // 上段の草地を描き終えた後に水流を描く。これで地面に隠れない。
     drawUpperExpressCurrent();
   ...
   if(raceRouteOpen()){
     ...
     if(cloudRaceWon){
       ...
       if(!vegBoss.dead){
         ...
         // 巨大カボチャ＋顔＋大包丁。包丁は常にプレイヤーがいる左右へ持ち替える。
         ...
         // ハロウィン風の三角目＋ギザギザ口
         ctx.fillStyle='#3b2415';ctx.strokeStyle='#111';ctx.lineWidth=4;
         ctx.beginPath();ctx.moveTo(-38,-24);ctx.lineTo(-10,-30);ctx.lineTo(-22,-4);ctx.closePath();ctx.fill();ctx.stroke();
         ctx.beginPath();ctx.moveTo(38,-24);ctx.lineTo(10,-30);ctx.lineTo(22,-4);ctx.closePath();ctx.fill();ctx.stroke();
         ctx.beginPath();ctx.moveTo(-34,16);ctx.lineTo(-18,8);ctx.lineTo(-5,19);ctx.lineTo(8,8);ctx.lineTo(23,18);ctx.lineTo(36,10);ctx.lineTo(28,31);ctx.lineTo(-27,31);ctx.closePath();ctx.fill();ctx.stroke();
         const ws=vegBoss.weaponSide||-1;
         ctx.save();ctx.scale(ws,1);ctx.rotate(vegBoss.swing>0?-1.12:.26);
       ...
       if(crystalBossDefeated){
        const rb=lateRainbows[0],cols=['#f44','#f90','#fd4','#5c5','#49f','#85f'];ctx.save();ctx.lineCap='round';for(let i=0;i<6;i++){ctx.strokeStyle=cols[i];ctx.lineWidth=rb.w/6+3;ctx.beginPath();ctx.moveTo(rb.x1,rb.y1+(i-2.5)*rb.w/6);ctx.lineTo(rb.x2,rb.y2+(i-2.5)*rb.w/6);ctx.stroke();}ctx.restore();
       }
       if(lightBossDefeated){
        const rb=lateRainbows[1],cols=['#f44','#f90','#fd4','#5c5','#49f','#85f'];ctx.save();ctx.lineCap='round';for(let i=0;i<6;i++){ctx.strokeStyle=cols[i];ctx.lineWidth=rb.w/6+3;ctx.beginPath();ctx.moveTo(rb.x1,rb.y1+(i-2.5)*rb.w/6);ctx.lineTo(rb.x2,rb.y2+(i-2.5)*rb.w/6);ctx.stroke();}ctx.restore();
        ...
        for(const e of rushMiniBosses){if(e.dead)continue;ctx.save();ctx.translate(e.x,e.y);const c=e.type==='melon'?'#7acb65':e.type==='berry'?'#b65ac8':e.type==='pine'?'#d9a63e':'#ef9b43';circle(0,0,e.r,c,'#111',6);
          circle(-14,-9,6,'#fff','#111',3);circle(14,-9,6,'#fff','#111',3);circle(-13,-8,2.5,'#111','#111',1);circle(13,-8,2.5,'#111','#111',1);
          ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-20,-22);ctx.lineTo(-7,-17);ctx.moveTo(20,-22);ctx.lineTo(7,-17);ctx.moveTo(-14,15);ctx.quadraticCurveTo(0,23,14,15);ctx.stroke();
          const side=e.side||-1;ctx.save();ctx.scale(side,1);const ex=e.thrustT>0?50:0;line(25,12,94+ex,12,11,'#111');line(25,12,94+ex,12,5,'#e5edf0');ctx.fillStyle='#eef8fb';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(92+ex,12);ctx.lineTo(107+ex,2);ctx.lineTo(132+ex,12);ctx.lineTo(107+ex,22);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
          ctx.fillStyle='#fff';ctx.font='900 12px system-ui';ctx.textAlign='center';ctx.fillText(e.name,0,-e.r-10);ctx.restore();}
        for(const p of bananaPeels){if(p.used)continue;ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle='#ffe44d';ctx.strokeStyle='#111';ctx.lineWidth=4;
       ...
       if(player.lightStaff||bananaBossDefeated){
        // 飛行ルート：矢印→小島→最終丸島。橋は置かず、光翼で渡る。
        for(const q of finalSkyRoute.stepping){ctx.fillStyle='#9ed77c';ctx.strokeStyle='#111';ctx.lineWidth=7;ctx.beginPath();ctx.arc(q.x,q.y,q.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='#74543a';ctx.beginPath();ctx.moveTo(q.x-q.r*.72,q.y+q.r*.56);ctx.lineTo(q.x+q.r*.72,q.y+q.r*.56);ctx.lineTo(q.x+q.r*.38,q.y+q.r*.95);ctx.lineTo(q.x-q.r*.38,q.y+q.r*.95);ctx.closePath();ctx.fill();}
        for(const ar of finalSkyRoute.arrows){ctx.save();ctx.translate(ar.x,ar.y);ctx.rotate(ar.a);ctx.globalAlpha=.72;ctx.fillStyle='#fff';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(34,0);ctx.lineTo(-16,-18);ctx.lineTo(-5,0);ctx.lineTo(-16,18);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}
        const fa=finalSkyRoute.arena;ctx.fillStyle='#8dcf70';ctx.strokeStyle='#111';ctx.lineWidth=10;ctx.beginPath();ctx.arc(fa.x,fa.y,fa.r,0,Math.PI*2);ctx.fill();ctx.stroke();
        ctx.fillStyle='#70513a';ctx.beginPath();ctx.arc(fa.x,fa.y+16,fa.r*.93,0,Math.PI);ctx.fill();
        for(const q of finalSkyRoute.safe){ctx.fillStyle='#b2df87';ctx.strokeStyle='#111';ctx.lineWidth=8;ctx.beginPath();ctx.arc(q.x,q.y,q.r,0,Math.PI*2);ctx.fill();ctx.stroke();}
        if(finalBoss.active&&!finalBoss.dead&&finalBoss.pulseWarn>0){
          const p=1-finalBoss.pulseWarn/1.25;ctx.save();ctx.globalAlpha=.18+.34*p;ctx.fillStyle='#fff2a0';ctx.beginPath();ctx.arc(fa.x,fa.y,fa.r-8,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=12;ctx.beginPath();ctx.arc(fa.x,fa.y,(fa.r-30)*(0.35+0.65*p),0,Math.PI*2);ctx.stroke();ctx.restore();
        }
        if(!finalBoss.dead){const b=finalBoss;ctx.save();ctx.translate(b.x,b.y);if(b.flash>0)ctx.globalAlpha=.55;ctx.globalCompositeOperation='source-over';circle(0,0,86,'#fff3a8','#111',8);circle(0,0,57,'#e7d06b','#80691d',6);circle(-20,-10,7,'#111','#111',1);circle(20,-10,7,'#111','#111',1);ctx.strokeStyle='#111';ctx.lineWidth=7;ctx.beginPath();ctx.arc(0,18,24,.1,Math.PI-.1);ctx.stroke();for(let i=0;i<8;i++){ctx.save();ctx.rotate(i*Math.PI/4+(performance.now()*.0005));ctx.fillStyle='#fff9d0';ctx.strokeStyle='#111';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(62,-12);ctx.lineTo(112,0);ctx.lineTo(62,12);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}ctx.restore();ctx.fillStyle='#111';ctx.font='900 18px system-ui';ctx.textAlign='center';ctx.fillText('天空核',b.x,b.y-120);ctx.fillRect(b.x-100,b.y+105,200,14);ctx.fillStyle='#fff';ctx.fillRect(b.x-95,b.y+109,190*Math.max(0,b.hp/b.maxHp),6);}
       }
       if(crystalBossDefeated){
        ...
        for(const v of ultraVines){if(v.dead)continue;ctx.save();ctx.translate(v.x,v.y);ctx.rotate(v.a||0);const L=v.len||210,wave=Math.sin(v.phase)*12;ctx.strokeStyle='#175f32';ctx.lineWidth=26;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-L/2,0);ctx.bezierCurveTo(-L/6,-30+wave,L/6,30-wave,L/2,0);ctx.stroke();ctx.strokeStyle='#55bd58';ctx.lineWidth=14;ctx.stroke();
          // 両側に大きなトゲを規則的に生やす。
          for(let k=-4;k<=4;k++){const x=k*L/9,y=Math.sin((k+4)/8*Math.PI*2)*10,side=k%2?1:-1;ctx.fillStyle='#7b2c51';ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x-9,y);ctx.lineTo(x,y+side*30);ctx.lineTo(x+9,y);ctx.closePath();ctx.fill();ctx.stroke();}
          ctx.restore();}
       ...
       if(fruitSpearBossDefeated){
         ...
         for(const r of fruitRoadGeo.path){
           ...
           ctx.beginPath();ctx.roundRect(r.x+10,r.y+24,r.w-20,r.h+36,34);ctx.fill();ctx.stroke();
           ctx.fillStyle='#7bc766';ctx.strokeStyle='#111';ctx.lineWidth=7;
           ctx.beginPath();ctx.roundRect(r.x,r.y,r.w,r.h,34);ctx.fill();ctx.stroke();
           ctx.save();ctx.globalAlpha=.22;ctx.strokeStyle='#f2e6a8';ctx.lineWidth=5;
           ctx.beginPath();ctx.moveTo(r.x+35,r.y+r.h/2);ctx.lineTo(r.x+r.w-35,r.y+r.h/2);ctx.stroke();ctx.restore();
         }
         // 固定砲台の花は空中に直置きせず、必ず小足場か雲に乗せる。
         for(const p of roadFlowerPads){
           ctx.save();ctx.translate(p.x,p.y+22);
           if(p.cloud){
             ctx.fillStyle='#f7fbff';ctx.strokeStyle='#111';ctx.lineWidth=5;
             for(const [cx,cy,cr] of [[-25,4,24],[0,-5,30],[28,5,23]]){ctx.beginPath();ctx.arc(cx,cy,cr,0,Math.PI*2);ctx.fill();ctx.stroke();}
           }else{
             ctx.fillStyle='#765638';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.ellipse(0,12,52,28,0,0,Math.PI*2);ctx.fill();ctx.stroke();
             ctx.fillStyle='#79c766';ctx.beginPath();ctx.ellipse(0,0,52,25,0,0,Math.PI*2);ctx.fill();ctx.stroke();
           }
           ctx.restore();
         }
         for(const f of roadFlowers){
           if(f.dead)continue;ctx.save();ctx.translate(f.x,f.y);if(f.flash>0)ctx.globalAlpha=.55;
           line(0,8,0,34,10,'#111');line(0,8,0,34,5,'#4a9443');
           for(let i=0;i<8;i++){const a=i*Math.PI/4;circle(Math.cos(a)*20,Math.sin(a)*20-7,11,'#f5d64d','#111',3)}
           circle(0,-7,12,'#6e4a2d','#111',4);circle(-4,-9,2,'#111','#111',1);circle(4,-9,2,'#111','#111',1);
           ctx.restore();
         }
         for(const e of roadVegSoldiers){
           if(e.dead)continue;ctx.save();ctx.translate(e.x,e.y);if(e.flash>0)ctx.globalAlpha=.55;
           const side=player.x<e.x?-1:1;
           if(e.type==='carrotSword'){
             ctx.fillStyle='#ef843b';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-18,-20);ctx.lineTo(20,-20);ctx.lineTo(5,30);ctx.lineTo(-7,30);ctx.closePath();ctx.fill();ctx.stroke();
             line(-8,-22,-18,-42,7,'#3e9c49');line(5,-22,15,-43,7,'#4fb455');
             ctx.save();ctx.scale(side,1);ctx.rotate((e.swingT||0)>0?-.9:.25);line(20,0,64,0,10,'#111');line(20,0,64,0,5,'#e8edf0');ctx.restore();
           }else if(e.type==='leekSpear'){
             ctx.fillStyle='#eaf4d5';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.ellipse(0,3,18,34,0,0,Math.PI*2);ctx.fill();ctx.stroke();line(0,-26,0,-48,9,'#42a54b');
             ctx.save();ctx.scale(side,1);ctx.rotate((e.swingT||0)>0?.18:0);line(18,0,88,0,9,'#111');line(18,0,88,0,4,'#dce7ea');ctx.restore();
           }else{
             circle(0,2,30,'#e8e5cf','#111',5);line(-10,-25,-20,-45,8,'#4a9f48');line(8,-25,18,-46,8,'#4a9f48');
             ctx.save();ctx.scale(side,1);ctx.rotate((e.swingT||0)>0?-.72:.35);line(24,0,58,0,12,'#111');circle(67,0,18,'#8e8e8e','#111',5);ctx.restore();
           }
           ctx.restore();
         }
         for(const b of fruitMiniBosses){
           if(b.dead)continue;ctx.save();ctx.translate(b.x,b.y);if(b.flash>0)ctx.globalAlpha=.55;
           if(b.type==='apple'){
             circle(0,0,46,'#e64a43','#111',6);line(0,-43,8,-63,9,'#705036');
             ctx.fillStyle='#63ae50';ctx.strokeStyle='#111';ctx.lineWidth=4;ctx.beginPath();ctx.ellipse(18,-52,18,9,.4,0,Math.PI*2);ctx.fill();ctx.stroke();
           }else if(b.type==='orange'){
             circle(0,0,49,'#f39a31','#111',6);ctx.strokeStyle='#cf7427';ctx.lineWidth=3;
             for(let i=0;i<7;i++){const a=i*Math.PI*2/7;line(Math.cos(a)*20,Math.sin(a)*20,Math.cos(a)*42,Math.sin(a)*42,3,'#cf7427');}
           }else{
             for(const [gx,gy] of [[0,-28],[-22,-10],[22,-10],[-32,14],[0,14],[32,14],[-18,38],[18,38]])circle(gx,gy,21,'#7850a5','#111',5);
             line(0,-47,6,-68,8,'#4b8f45');
           }
           // ちゃんと表情のある中ボス顔。
           circle(-15,-10,6,'#fff','#111',3);circle(15,-10,6,'#fff','#111',3);
           circle(-13,-9,2.8,'#111','#111',1);circle(13,-9,2.8,'#111','#111',1);
           ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-21,-23);ctx.lineTo(-7,-18);ctx.moveTo(21,-23);ctx.lineTo(7,-18);ctx.stroke();
           ctx.beginPath();ctx.arc(0,13,16,.15,Math.PI-.15);ctx.stroke();
           // 全員、長い槍で突いてくる。
           const side=b.side||-1;ctx.save();ctx.scale(side,1);const ex=b.thrustT>0?48:0;
           line(28,12,98+ex,12,11,'#111');line(28,12,98+ex,12,5,'#dbe7ec');
           ctx.fillStyle='#eaf5fa';ctx.strokeStyle='#111';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(94+ex,12);ctx.lineTo(109+ex,2);ctx.lineTo(132+ex,12);ctx.lineTo(109+ex,22);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
           ctx.fillStyle='#111';ctx.font='900 14px system-ui';ctx.textAlign='center';ctx.fillText(b.name,0,-76);
           ctx.fillRect(-54,64,108,10);ctx.fillStyle='#fff';ctx.fillRect(-50,67,100*Math.max(0,b.hp/b.maxHp),4);
           ctx.restore();
         }
         if(!fruitSpearBoss.dead){
           const b=fruitSpearBoss;ctx.save();ctx.translate(b.x,b.y);if(b.flash>0)ctx.globalAlpha=.55;
           // パイナップル鎧の槍ボス
           ctx.fillStyle='#e7b63f';ctx.strokeStyle='#111';ctx.lineWidth=7;ctx.beginPath();ctx.ellipse(0,0,48,62,0,0,Math.PI*2);ctx.fill();ctx.stroke();
           ctx.strokeStyle='#b7832d';ctx.lineWidth=3;for(let yy=-38;yy<=38;yy+=20){line(-38,yy,38,yy,3,'#b7832d');}
           for(let xx=-28;xx<=28;xx+=19){line(xx,-48,xx,48,3,'#b7832d');}
 ...
 for(const fx of airMagicImpacts){
   ...
   if(fx.kind==='fire'){
     ...
     for(let i=0;i<6;i++){
       const a=i*Math.PI/3,ex=Math.cos(a)*R,ey=Math.sin(a)*R;
       line(0,0,ex,ey,8,'#e9fbff');
       const bx=Math.cos(a)*R*.58,by=Math.sin(a)*R*.58;
       line(bx,by,bx+Math.cos(a+.72)*R*.25,by+Math.sin(a+.72)*R*.25,5,'#e9fbff');
       line(bx,by,bx+Math.cos(a-.72)*R*.25,by+Math.sin(a-.72)*R*.25,5,'#e9fbff');
     }
     circle(0,0,17,'#dff8ff','#4aaed7',5);
   }
   ctx.restore();
 }
...
function drawPlayer(){
 ...
 if(player.shield&&(player.lightStaff||player.weapon===5)&&(player.shieldType===7||player.lightShield)){
   ...
   ctx.beginPath();ctx.moveTo(0,-74);ctx.quadraticCurveTo(72,-55,78,4);ctx.quadraticCurveTo(67,72,0,94);ctx.quadraticCurveTo(-67,72,-78,4);ctx.quadraticCurveTo(-72,-55,0,-74);ctx.closePath();ctx.fill();ctx.stroke();
   ctx.globalAlpha=.38+pulse*.10;ctx.strokeStyle='#e8c43d';ctx.lineWidth=5;ctx.stroke();
   ctx.globalAlpha=.18;ctx.strokeStyle='#fff';ctx.lineWidth=18;ctx.stroke();
   ctx.restore();
 }
 ctx.restore();
}
...
function drawWeapon(hx,hy,a,ext=0){const w=player.weapon;ctx.save();ctx.translate(hx+Math.cos(a)*ext,hy+Math.sin(a)*ext);ctx.rotate(a);ctx.lineCap='round';if(w===0){
   if(player.flameSword){
     // 炎神の剣：槍に見えないよう、短い柄＋幅広い炎型の刀身。
     line(-5,0,13,0,13,'#111');line(-5,0,13,0,6,'#7b3d27');
     line(11,-15,11,15,8,'#111');line(11,-10,11,10,4,'#f0a33b');
     ctx.fillStyle='#ff5528';ctx.strokeStyle='#111';ctx.lineWidth=6;ctx.lineJoin='round';
     ctx.beginPath();ctx.moveTo(15,-11);ctx.lineTo(39,-16);ctx.lineTo(55,-10);ctx.lineTo(72,-21);ctx.lineTo(65,-5);ctx.lineTo(88,0);ctx.lineTo(65,7);ctx.lineTo(73,21);ctx.lineTo(53,12);ctx.lineTo(38,17);ctx.lineTo(15,10);ctx.closePath();ctx.fill();ctx.stroke();
     ctx.strokeStyle='#ffcf52';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(25,0);ctx.lineTo(67,0);ctx.lineTo(78,-1);ctx.stroke();
     ctx.strokeStyle='#fff0a0';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(29,-5);ctx.lineTo(56,-8);ctx.stroke();
   }else{
     line(0,0,45,0,11,'#111');line(0,0,45,0,5,player.swordPlus?'#c8f2d2':'#eef5fa');
     if(player.swordPlus)line(10,-2,41,-2,2,'#64c58b');
     line(5,-11,5,11,7,'#111');line(5,-7,5,7,3,player.swordPlus?'#6fbd78':'#d8a93d');
   }
 }
 else if(w===1){
   ...
   if(w===5){
     line(-3,0,48,0,11,'#111');line(-3,0,48,0,6,'#fff9dc');
     line(4,-2,43,-2,2,'#f2cf55');
     circle(54,0,15,'#fff7b0','#111',5);circle(54,0,8,'#ffffff','#d7b53c',3);
     for(let i=0;i<4;i++){const aa=i*Math.PI/2;line(54+Math.cos(aa)*16,Math.sin(aa)*16,54+Math.cos(aa)*25,Math.sin(aa)*25,3,'#fff4a1');}
   }else{
     line(-2,0,45,0,10,'#111');line(-2,0,45,0,5,'#6d3e2a');circle(50,0,11,w===3?'#ff5a4f':'#69c9ff','#111',5);circle(50,0,4,'#fff','#111',2);
   }
 }ctx.restore()}
...
function drawStaffSkillEffects(){
 ...
 if(((player.skillKind==='lightWing'&&player.skillT>0)||postGameAngel)&&player.face!=='up'){
   ...
   if(Math.abs(dy)>Math.abs(dx)){angelWing(-1);angelWing(1);}
   else angelWing(dx>0?-1:1);
   ctx.restore();
 }else if(player.skillKind==='fire'){
...
function drawEndingOverlay(){
 ...
 const fade=clamp(endingT/1.15,0,1);
 ctx.save();
 ctx.globalAlpha=.86*fade;ctx.fillStyle='#18334a';ctx.fillRect(0,0,canvas.width,canvas.height);
 ctx.globalAlpha=fade;ctx.textAlign='center';
 const cx=canvas.width/2,cy=canvas.height/2;
 ctx.fillStyle='#fff7b0';ctx.strokeStyle='#111';ctx.lineWidth=7;
 ctx.font=`900 ${Math.max(34,Math.min(58,canvas.width*.07))}px system-ui`;
 ctx.strokeText('FINAL CLEAR!',cx,cy-92);ctx.fillText('FINAL CLEAR!',cx,cy-92);
 ctx.fillStyle='#fff';ctx.font=`900 ${Math.max(22,Math.min(34,canvas.width*.042))}px system-ui`;
 ctx.fillText('盾を構えとけば何とか凌げる',cx,cy-22);
 ctx.fillStyle='#dff7ff';ctx.font=`700 ${Math.max(16,Math.min(23,canvas.width*.028))}px system-ui`;
 ctx.fillText('最後まで凌ぎきった。',cx,cy+28);
 ctx.fillStyle='#fff7b0';ctx.font=`900 ${Math.max(26,Math.min(40,canvas.width*.05))}px system-ui`;
 ctx.fillText('THE END',cx,cy+94);
 ctx.restore();
}
let last=performance.now();function loop(t){let dt=Math.min(.033,(t-last)/1000);last=t;update(dt);drawWorld();drawCloudRaceIntroOverlay();drawEndingOverlay();requestAnimationFrame(loop)}requestAnimationFrame(loop);
